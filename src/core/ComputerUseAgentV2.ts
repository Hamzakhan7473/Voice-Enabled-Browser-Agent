...
import { Action, PageCapture, bbCapture, bbClick, bbGoto, bbScroll, bbType, bbWaitFor } from "../browser/bb-exec.js";
...
export class ComputerUseAgentV2 {
  ...
  async runAgent(goal: string): Promise<string> {
  ...
  private async decide(goal: string, perception: PageCapture): Promise<Action[]> {
    ...
    switch (action.name) {
      ...
      case "scroll":
        await bbScroll(this.page, {
          dx: action.dx,
          dy: action.dy,
          to: action.scroll_to as "top" | "bottom" | undefined
        });
        break;
      ...
      default:
        console.warn(`⚠️ Unknown action: ${(action as any).name}`);
    }
  }
}
